...
object RoutingTable {
  ...
  def insert(bucket: RoutingTable, node: NodeInfo): RoutingTable = bucket match {
    ...
    case Bucket(from, until, nodes) =>
      if (nodes.size == MaxNodes) {
        ...
        val splitBucket: RoutingTable = Split(center, Bucket(from, center - 1, Map.empty), Bucket(center, until, Map.empty))
  ...
  def remove(bucket: RoutingTable, nodeId: NodeId): RoutingTable = bucket match {
    case b@Split(center, lower, higher) =>
  ...
  def findBucket(bucketTree: RoutingTable, nodeId: NodeId): Bucket = bucketTree match {
